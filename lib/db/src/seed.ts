...
export async function ensureSeedData() {
  ...
  await db.insert(deadlines).values([
    { userId: user.id, type: "IRB renewal", title: "Human Subjects Protocol 24-118", dueDate: new Date("2026-09-04"), progress: 72, status: "due_soon" },
    { userId: user.id, type: "Funding report", title: "NSF CAREER annual progress report", dueDate: new Date("2026-10-15"), progress: 38, status: "on_track" },
    { userId: user.id, type: "Data management", title: "NIH Data Management & Sharing update", dueDate: new Date("2026-08-27"), progress: 12, status: "attention" },
  ]);
  await db.insert(activities).values([
    { userId: user.id, kind: "flagged", title: "Retraction signal caught: STAP Stem Cell Study", description: "Retraction Watch identified direct retraction notice for 10.1038/nature13358 (Obokata et al. 2014). Paper quarantined.", tone: "danger" },
    { userId: user.id, kind: "escalation", title: "Propagation risk routed to PI: Lin et al. (2015)", description: "Semantic Scholar 1-hop graph found reference to retracted DOI 10.1038/nature13358. Escalated for human claim evaluation.", tone: "warning" },
    { userId: user.id, kind: "scan", title: "Guardian full desk sweep completed", description: "Scanned 5 citations using Crossref, Retraction Watch, and Semantic Scholar graph traversal. Strands Agent trace recorded.", tone: "neutral" },
  ]);
  return user.id;
}